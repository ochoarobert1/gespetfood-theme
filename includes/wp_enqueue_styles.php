...
function gespetfood_load_css() {
    ...
    if (!is_admin()){
        if ($_SERVER['REMOTE_ADDR'] == '::1') {
            ...
            /*- SWIPER ON LOCAL -*/
            wp_register_style('swiper-css', get_template_directory_uri() . '/css/swiper.min.css', false, '4.5.3', 'all');
        ...
        /*- GOOGLE FONTS -*/
        wp_register_style('google-fonts', 'https://fonts.googleapis.com/css?family=Montserrat:100,200,300,400,500,600,700,800,900&display=swap', false, $version_remove, 'all');
        wp_enqueue_style('google-fonts');

        /*- MAIN STYLE -*/
        wp_register_style('main-style', get_template_directory_uri() . '/css/gespetfood-style.css', false, $version_remove, 'all');
        wp_enqueue_style('main-style');

        /*- MAIN MEDIAQUERIES -*/
        wp_register_style('main-mediaqueries', get_template_directory_uri() . '/css/gespetfood-mediaqueries.css', array('main-style'), $version_remove, 'all');
...
add_action('wp_enqueue_scripts', 'gespetfood_load_css');
